...
export default function formatter(document: TextDocument): TextEdit[] {
    ...
    const edits = lines.map(line => {
        ...
        // Indentation
        if (!line.label && line.directive && !['.data', '.text', '.macro', '.end_macro'].includes(line.directive)) result = result.padEnd(longestLabel, ' ');
        ...
        if (line.arguments) {
            line.arguments.forEach((arg, index) => {
                if (line.arguments && index !== line.arguments.length - 1) {
